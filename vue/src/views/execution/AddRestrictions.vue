                  ...
                  :style="{ 'min-height': `${heigthDiv}px`
                    ...
                    >
                      <template #default="{ tr, id }">
                        <DataTableRestriccionesRow :listindex="[index1,index2,id]" :restriction_data="tr" :isupdate="tr.isupdate" :frontId="frente.codFrente" :phaseId="fase.codFase" :hideCols="[]" @openModal="openModal" @updateRow = "updateRow" @RegistrarCambioRow = "RegistrarCambioRow"   />
                      </template>
...
export default {
  ...
  mounted: function() {
    ...
    store.dispatch('getNameProy').then((response) => {
      this.nameProyecto = response
    });

    store.dispatch('get_datos_restricciones').then((response) => {
      this.filterSectionHeight()
