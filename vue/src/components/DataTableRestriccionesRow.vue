<template>
  <tr @click="visualizar">
    ...
    <td> <input @keyup="verificarCambio({name:'desActividad', value: restriction_row.desActividad})" v-model="restriction_row.desActividad" type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded  " /> </td>
    <td> <input @keyup="verificarCambio({name:'desRestriccion', value: restriction_row.desRestriccion})" v-model="restriction_row.desRestriccion" type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded  "  /> </td>
    <td>
      <div class="flex relative  cursor-pointer cursor-pointer">
        <input type="hidden" v-model="restriction_row.codTipoRestriccion">
        <input v-model="restriction_row.desTipoRestriccion" type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded "  placeholder="Elegir Tipo Restricción" />
    ...
    <td>
      ...
      <v-date-picker  @click="verificarCambio({name:'dayFechaRequerida', value: restriction_row.dayFechaRequerida})"  ref="datepicker"  v-model="restriction_row.dayFechaRequerida" mode="date" is24hr: false class="flex relative  cursor-pointer cursor-pointer" >
          <template v-slot="{ inputValue, inputEvents }" >
            <input
              class="w-full border border-[#8A9CC9] px-2 h-10 rounded "
    ...
    <td>
      ...
      <v-date-picker  @click="verificarCambio({name:'dayFechaConciliada', value: restriction_row.dayFechaConciliada})"  ref="datepicker"  v-model="(restriction_row.dayFechaConciliada)" mode="date" is24hr: false class="flex relative  cursor-pointer cursor-pointer" >
          <template v-slot="{ inputValue, inputEvents }" >
            <input
              class="w-full border border-[#8A9CC9] px-2 h-10 rounded "
              placeholder="Fecha de inicio*"
    ...
    <td>
      <div class="flex relative  cursor-pointer cursor-pointer">
        ...
        <input v-model="restriction_row.desUsuarioResponsable" type="text" class="w-full border border-[#8A9CC9] px-2 h-10 rounded "  placeholder="Elegir Tipo Restricción" />
    ...
    <td >{{ restriction_row.desAreaResponsable}}</td>
    <td>
      <div class="flex relative  cursor-pointer cursor-pointer">
...
export default {
  ...
  methods: {
    ...
    verificarCambio(data) {
        ...
        console.log("1 => input ====> "+String(input)+" || "+"comparar ====>"+String(comparar))

        // console.log(this.restriction_data['dayFechaRequerida'])
        ...
        console.log("2 => input ====> "+String(input)+" || "+"comparar ====>"+String(comparar))
